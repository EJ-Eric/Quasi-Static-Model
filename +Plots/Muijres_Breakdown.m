...
torque_list = ["Torque_Lift","Torque_Drag","Torque_Rotation","Torque_AM","Torque_Inertia"];%,"Torque_Total"];
force_list  = ["Force_Lift","Force_Drag","Force_Rotation","Force_AM"];%,"Force_Total"];
...
hold on
idx = 2;
for torque = torque_list
    p = plot(...
        time_normalized,...
        (Fly.Dynamics.Frame_Body.LH.(torque)(idx,:) + ...
            Fly.Dynamics.Frame_Body.RH.(torque)(idx,:)) / ...
            (Fly.Morphology.total.weight * meanWingLen),...
        DisplayName=replace(torque,"_","\_"));
    if torque == "Torque_Total",p.Color="k";end
end
...
hold on
idx = 3;
for force = force_list
    p = plot(...
        time_normalized,...
        (Fly.Dynamics.Frame_Body.LH.(force)(idx,:) + ...
            Fly.Dynamics.Frame_Body.RH.(force)(idx,:)) / ...
            (Fly.Morphology.total.weight),...
        DisplayName=replace(force,"_","\_"));
    if force == "Force_Total",p.Color="k";end
end
